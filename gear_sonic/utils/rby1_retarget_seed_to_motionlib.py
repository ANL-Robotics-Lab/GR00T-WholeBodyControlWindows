...
Example: convert already-retargeted RBY1 CSVs
    python gear_sonic/data_process/rby1_retarget_seed_to_motionlib.py \
        ...
        --fps-source 30 --fps 30 \
        --individual --recursive

Example: make a slowed, wheel-feasible training clip from a GEM-X RBY1 CSV
    python gear_sonic/data_process/rby1_retarget_seed_to_motionlib.py \
        --input /path/to/dance.csv --output /path/to/dance_trainable.pkl \
        --input-format rby1_csv --input-joint-order mujoco \
        --fps-source 9 --fps 9 \
        --rby1-root-mode wheel_feasible_planar_yaw_relative \
        --rby1-joint-smoothing-window 7 --wheel-sign -1

Using the same lower value for ``--fps-source`` and ``--fps`` retains every
CSV frame and stretches playback time; it does not discard frames.
"""
...
from gear_sonic.utils.rby1_order import (
    ...
    RBY1_MUJOCO_DOF_NAMES as RBY1_MJ_DOF_NAMES,
)
...
def _moving_average_1d(x: np.ndarray, window: int) -> np.ndarray:
    if window <= 1:
        return x
    if window % 2 == 0:
        raise ValueError("Smoothing windows must be odd when greater than one.")
    pad = window // 2
...
def _smooth_joint_trajectory(dof: np.ndarray, window: int) -> np.ndarray:
    """Apply a centered moving average independently to each joint."""
    if window < 1 or (window > 1 and window % 2 == 0):
        raise ValueError("--rby1-joint-smoothing-window must be a positive odd integer.")
    if window == 1:
        return np.asarray(dof, dtype=np.float32)
    smoothed = np.stack(
        [_moving_average_1d(dof[:, index], window) for index in range(dof.shape[1])],
        axis=1,
    )
    return smoothed.astype(np.float32)


def _wheel_feasible_planar_root(
    dof_mj_rad: np.ndarray,
    *,
    ...
    """Reconstruct a non-holonomic root path from differential wheel angles.

    GEM-X derives wheel angles by projecting each source root displacement onto
    the previous base heading. Integrating those same wheel deltas recovers the
    realizable forward/yaw component and intentionally drops lateral sliding.
    ``wheel_sign`` maps exported CSV angles back to GEM-X's kinematic sign.
    """
    if wheel_radius <= 0.0 or wheel_track <= 0.0:
        raise ValueError("Wheel radius and track width must be positive.")
    if not math.isfinite(wheel_sign) or abs(wheel_sign) <= 1e-12:
        raise ValueError("Wheel sign must be finite and non-zero.")

    right_index = RBY1_MJ_DOF_NAMES.index("right_wheel")
    left_index = RBY1_MJ_DOF_NAMES.index("left_wheel")
    wheel_angles = (
        np.asarray(dof_mj_rad[:, [right_index, left_index]], dtype=np.float64)
        / wheel_sign
    )
    wheel_delta = np.diff(wheel_angles, axis=0)
    forward_delta = wheel_radius * 0.5 * (wheel_delta[:, 0] + wheel_delta[:, 1])
    yaw_delta = wheel_radius * (wheel_delta[:, 0] - wheel_delta[:, 1]) / wheel_track

    num_frames = dof_mj_rad.shape[0]
    root_pos = np.zeros((num_frames, 3), dtype=np.float64)
    root_pos[:, 2] = nominal_root_z
    yaw = np.zeros(num_frames, dtype=np.float64)
    for frame in range(1, num_frames):
        previous_yaw = yaw[frame - 1]
        root_pos[frame, 0] = (
            root_pos[frame - 1, 0] + forward_delta[frame - 1] * np.cos(previous_yaw)
        )
        root_pos[frame, 1] = (
            root_pos[frame - 1, 1] + forward_delta[frame - 1] * np.sin(previous_yaw)
        )
        yaw[frame] = previous_yaw + yaw_delta[frame - 1]

    return root_pos.astype(np.float32), _quat_wxyz_from_yaw(yaw)
...
def load_rby1_csv(
    ...
    *,
    ...
    root_translation_unit: Literal["auto", "m", "cm"] = "auto",
    dof_unit: Literal["auto", "rad", "deg"] = "auto",
    root_mode: Literal[
        "raw", "planar_yaw_relative", "wheel_feasible_planar_yaw_relative"
    ] = "raw",
    nominal_root_z: float = 0.0,
    joint_smoothing_window: int = 1,
    wheel_radius: float = 0.10,
    wheel_track: float = 0.53,
    wheel_sign: float = 1.0,
    ...
    Accepted root pose schemas:
      ...
      D) GEM upstream planar root (meters/radians):
         root_translateX, root_translateY, root_yaw
    ...
    planar_root_cols = {"root_translateX", "root_translateY", "root_yaw"}
    is_gem_planar_schema = planar_root_cols.issubset(df.columns)

    if {"root_x", "root_y", "root_z"}.issubset(df.columns):
        root_pos = df[["root_x", "root_y", "root_z"]].to_numpy(dtype=np.float32)
        inferred_root_unit = "m"
        ...
        inferred_root_unit = "cm"
    elif is_gem_planar_schema:
        root_xy = df[["root_translateX", "root_translateY"]].to_numpy(dtype=np.float32)
        root_pos = np.column_stack(
            (root_xy, np.full(T, nominal_root_z, dtype=np.float32))
        )
        inferred_root_unit = "m"
    else:
        raise ValueError(
            f"{csv_path}: expected root position columns "
            "root_x/root_y/root_z, root_translateX/root_translateY/root_translateZ, "
            "or planar root_translateX/root_translateY/root_yaw."
        )

    resolved_root_unit = (
        inferred_root_unit if root_translation_unit == "auto" else root_translation_unit
    )
    if resolved_root_unit == "cm":
        root_pos = root_pos / 100.0
    elif resolved_root_unit != "m":
        raise ValueError(f"Unknown root_translation_unit: {root_translation_unit}")
    ...
    if set(quat_cols).issubset(df.columns):
        ...
        root_quat_wxyz = _quat_wxyz_from_yaw(
            df["root_yaw"].to_numpy(dtype=np.float64)
        )
    else:
        raise ValueError(
            f"{csv_path}: expected root quaternion columns {quat_cols} or Euler columns "
            f"{euler_cols}, {bones_euler_cols}, or planar root_yaw."
        )

    if input_joint_order == "mujoco":
        ordered_joint_names = RBY1_MJ_DOF_NAMES
    elif input_joint_order == "isaaclab":
        ordered_joint_names = RBY1_ISAACLAB_DOF_NAMES
    else:
        raise ValueError(f"Unknown input_joint_order: {input_joint_order}")

    direct_cols = ordered_joint_names
    suffixed_cols = [f"{name}_dof" for name in ordered_joint_names]
    if set(direct_cols).issubset(df.columns):
        dof = df[direct_cols].to_numpy(dtype=np.float32)
        inferred_dof_unit = "rad"
    elif set(suffixed_cols).issubset(df.columns):
        dof = df[suffixed_cols].to_numpy(dtype=np.float32)
        # The GEM upstream planar serializer uses the `_dof` suffix for
        # radian-valued joints. The canonical Bones-style serializer uses the
        # same suffix for degrees, so the root schema disambiguates them.
        inferred_dof_unit = "rad" if is_gem_planar_schema else "deg"
    else:
    ...
    resolved_dof_unit = inferred_dof_unit if dof_unit == "auto" else dof_unit
    if resolved_dof_unit == "deg":
        dof = np.deg2rad(dof)
    elif resolved_dof_unit != "rad":
        raise ValueError(f"Unknown dof_unit: {dof_unit}")
    ...
    dof = _smooth_joint_trajectory(dof, joint_smoothing_window)

    if root_mode == "planar_yaw_relative":
        # The BVH/Open3D export uses its own pelvis/root frame. Replay the
        # planar path and heading relative to frame zero so RBY1 stays upright
        # with its wheeled base at the requested ground height.
        root_pos = root_pos.copy()
        planar_delta = root_pos[:, :2] - root_pos[0, :2]
        yaw = _yaw_from_quat_wxyz(root_quat_wxyz)
        # Express translation in the same frame as the zeroed heading.
        cos_yaw0 = np.cos(yaw[0])
        sin_yaw0 = np.sin(yaw[0])
        root_pos[:, 0] = cos_yaw0 * planar_delta[:, 0] + sin_yaw0 * planar_delta[:, 1]
        root_pos[:, 1] = -sin_yaw0 * planar_delta[:, 0] + cos_yaw0 * planar_delta[:, 1]
        root_pos[:, 2] = nominal_root_z
        root_quat_wxyz = _quat_wxyz_from_yaw(yaw - yaw[0])
    elif root_mode == "wheel_feasible_planar_yaw_relative":
        root_pos, root_quat_wxyz = _wheel_feasible_planar_root(
            dof,
    ...
    # An already-retargeted CSV is the source of truth. Clipping changes its
    # pose before IsaacLab sees it, so make that an explicit safety option.
    if clip_limits:
...
def process_one_csv(
    ...
    *,
    ...
    root_rot_order: Literal["wxyz", "xyzw"],
    individual: bool,
    rby1_root_translation_unit: Literal["auto", "m", "cm"],
    rby1_dof_unit: Literal["auto", "rad", "deg"],
    rby1_root_mode: Literal[
        "raw", "planar_yaw_relative", "wheel_feasible_planar_yaw_relative"
    ],
    rby1_nominal_root_z: float,
    rby1_joint_smoothing_window: int,
    clip_limits: bool,
) -> tuple[str, Path, int]:
    if input_format == "rby1_csv":
        motion = load_rby1_csv(
            ...
            root_translation_unit=rby1_root_translation_unit,
            dof_unit=rby1_dof_unit,
            root_mode=rby1_root_mode,
            nominal_root_z=rby1_nominal_root_z,
            joint_smoothing_window=rby1_joint_smoothing_window,
            wheel_radius=projection_cfg.wheel_radius,
            wheel_track=projection_cfg.wheel_track,
            wheel_sign=projection_cfg.wheel_sign,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Override source CSV frame rate. Defaults to 30 for rby1_csv and 120 "
            "for bones_g1_csv_projection."
        ),
    )
    ...
    p.add_argument(
        "--rby1-root-translation-unit",
        choices=["auto", "m", "cm"],
        default="auto",
        help="Translation unit for rby1_csv. Auto uses meters for root_x columns "
             "and centimeters for root_translateX columns.",
    )
    p.add_argument(
        "--rby1-dof-unit",
        choices=["auto", "rad", "deg"],
        default="auto",
        help="Joint unit for rby1_csv. Auto uses radians for direct joint names "
             "and degrees for *_dof columns.",
    )
    p.add_argument(
        "--rby1-root-mode",
        choices=[
            "raw",
            "planar_yaw_relative",
            "wheel_feasible_planar_yaw_relative",
        ],
        default="raw",
        help=(
            "Root transform for rby1_csv. wheel_feasible_planar_yaw_relative "
            "reconstructs a realizable differential-drive path from wheel angles "
            "and is recommended for training."
        ),
    )
    p.add_argument(
        "--rby1-nominal-root-z",
        type=float,
        default=0.0,
        help="Base Z for either planar relative RBY1 root mode, in meters.",
    )
    p.add_argument(
        "--rby1-joint-smoothing-window",
        type=int,
        default=1,
        help=(
            "Positive odd moving-average window for rby1_csv joint trajectories. "
            "Use 7 for the supplied trainable dance variant; 1 preserves the source exactly."
        ),
    )
    clip_group = p.add_mutually_exclusive_group()
    clip_group.add_argument(
        "--clip-limits",
        dest="clip_limits",
        action="store_true",
        help="Clip DOFs to clean-MJCF limits.",
    )
    clip_group.add_argument(
        "--no-clip-limits",
        dest="clip_limits",
        action="store_false",
        help="Preserve input DOFs even when they exceed the clean-MJCF limits.",
    )
    p.set_defaults(clip_limits=None)

    # Wheel parameters are used by both G1 projection and the wheel-feasible
    # root mode for already-retargeted RBY1 CSVs.
    ...
    p.add_argument(
        ...
        default=1.0,
        help=(
            "Wheel sign convention multiplier. The supplied GEM-X CSV metadata "
            "uses -1.0; projection mode normally uses 1.0."
        ),
    )
    p.add_argument(
        "--root-motion-scale",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if fps_source is None:
        fps_source = 30 if args.input_format == "rby1_csv" else 120
    clip_limits = args.clip_limits
    if clip_limits is None:
        # Projected G1 poses are heuristic and should respect RBY1 limits;
        # already-retargeted RBY1 CSVs are preserved exactly by default.
        clip_limits = args.input_format == "bones_g1_csv_projection"

    if fps_source <= 0 or args.fps <= 0:
    ...
    for csv_path in csvs:
        try:
            name, out_path, n_frames = process_one_csv(
                ...
                rby1_root_translation_unit=args.rby1_root_translation_unit,
                rby1_dof_unit=args.rby1_dof_unit,
                rby1_root_mode=args.rby1_root_mode,
                rby1_nominal_root_z=args.rby1_nominal_root_z,
                rby1_joint_smoothing_window=args.rby1_joint_smoothing_window,
                clip_limits=clip_limits,
            )
            if not args.individual:
                ...
                if args.input_format == "rby1_csv":
                    motion = load_rby1_csv(
                        ...
                        fps_source=fps_source,
                        input_joint_order=args.input_joint_order,
                        root_translation_unit=args.rby1_root_translation_unit,
                        dof_unit=args.rby1_dof_unit,
                        root_mode=args.rby1_root_mode,
                        nominal_root_z=args.rby1_nominal_root_z,
                        joint_smoothing_window=args.rby1_joint_smoothing_window,
                        wheel_radius=args.wheel_radius,
                        wheel_track=args.wheel_track,
                        wheel_sign=args.wheel_sign,
                        clip_limits=clip_limits,
